...
(id INTEGER PRIMARY KEY AUTOINCREMENT, documents INTEGER, time INTEGER, max_df INTEGER, terms INTEGER, date TIMESTAMP);"
...
for ((docs=min_documents; docs<=max_documents; docs+=documents_step)); do
	...
	for ((dfp=min_maxDFpercent; dfp<=max_maxDFpercent; dfp+=maxDFpercent_step)); do
		...
		for((k=min_k; k<=max_k; k+=k_step)); do
			...
			time=$(ttime)
			sqlite3 results.db "INSERT INTO mahout_kmeans_text(documents, k, terms, max_df, time, date )
			                    VALUES( $docs,  $k, $dimensions,  $dfp, $time, CURRENT_TIMESTAMP);"
