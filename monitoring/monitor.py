...
def collect_future_metrics():
    """
    Loads metrics that were previously stored in a json file if any. If not returns an empty dict
    :return:
    """
...
def store_future_metrics(metrics):
    """
    store the metrics in a json file so that they will be collected in the future
    :param metrics:
    :return:
    """
...
##############################################################
    ...
    with open(_PID_FILE, 'w+') as f: f.write(str(getpid()))
